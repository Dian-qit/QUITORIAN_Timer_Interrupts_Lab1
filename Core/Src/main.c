...
// Timer 2 Interrupt Callback - called every 1 second
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
    // Background Task 1: TIM2 @ 1 Hz (high priority)
    ...
    // Background Task 2: TIM3 @ 2 Hz (lower priority)
    if (htim->Instance == TIM3)
    {
        HAL_GPIO_TogglePin(LED_BLUE_GPIO_Port, LED_BLUE_Pin);
        task_display_ready = 1;  // tell foreground to update display
    }
}
...
int main(void)
{
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  // Task A: ADC processing (triggered by TIM2 every 1s)
		  if (task_adc_ready)
		  {
			  HAL_GPIO_WritePin(DEBUG_PIN_GPIO_Port, DEBUG_PIN_Pin, GPIO_PIN_SET);
			  task_adc_ready = 0;
	      ...
	      // Task B: Display update (triggered by TIM3 every 0.5s)
	      if (task_display_ready)
	      {
	          task_display_ready = 0;
	          HAL_Delay(20);   // simulate 20ms display update
	      }

	      // Task C: 10-second periodic monitoring
	      if (tick_count >= 10)
	      {
	          tick_count = 0;
	          // perform 10-second task here
	      }
  }
  /* USER CODE END 3 */
}
